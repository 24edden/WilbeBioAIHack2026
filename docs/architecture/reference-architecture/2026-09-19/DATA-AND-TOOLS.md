...
Audited source-machine data root: `<LOCAL_PROJECT>/outputs/dataset-acquisition/`.
...
**Present:** local biotech reference graph at `<REFERENCE_MODEL_WORKSPACE>/graph/model/graph.json`, reference model `0.3.0-draft`, schema `1.2.0`. Selected source nodes and hash are preserved in `references/graph-reference-extract.json`.
