...
class Element {
 constructor(){this.children=[];this.value='';this.files=[];this.dataset={};this.classList={add:()=>{},remove:()=>{}};}
...
const elements=Object.fromEntries(['composer','question','files','submit','attach','demo','composer-error','attachments','drop-hint','source-note'].map(x=>[x,new Element()]));
let draft,action;
render({parentElement:{querySelector:q=>elements[q.slice(1)]},key:'test',data:{generation:'one',question:'Test question',files:[],extensions:['csv'],readOnly:false,disabled:false},setStateValue:(k,v)=>draft=v,setTriggerValue:(k,v)=>action=v});
let prevented=false;
